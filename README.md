# Vyapaar Saathi — merchant portal MVP

Agentic AI business partner for Paytm merchants (Team Yantrika deck): **understand → advise → execute**, in the merchant's language, with a human approving every action. One central portal, one Node service, no external workflow engine.

```
 DATA IN            MERCHANT BRAIN          AI CORE                 ACTION LAYER (built in)      PORTAL / CHANNELS
 txns, items,  ──►  Merchant-360 features ─► 5 agents + orchestrator ─► executor + rail adapters ─► React portal (sky + white)
 settlements,       Cognee memory graph      opportunity ranker          guardrails, undo, audit    voice · WhatsApp webhook
...
```bash
npm install
npm run build      # builds the React portal (web/) into public/
npm start          # http://localhost:3000  — portal + API, auto-seeds 6 synthetic merchants
...
Sections: **Overview** (today vs usual, 30-day bars with the weak weekday highlighted, KPIs, top action) · **Actions** (ranked proposals, how the agents agreed, approve/undo, live activity) · **Saathi** (chat + voice) · **Agents** · **Insights** (stock cover, customers, cash & GST, holdout impact, memory) · **Privacy & system** (consent switches, executor/Cognee/LLM/WhatsApp status, outbox, audit trail).
...
`server/executor.js`: each action type maps to a rail adapter. Approve returns `executing` immediately; the rail runs asynchronously, re-checks guardrails (spend cap, discount, audience, loan ≤ eligibility), then the action becomes `done` or `failed` (never hangs). Rails are mocks by default (`via: rail:mock`). To go live, set e.g. `RAIL_LAUNCH_OFFER_URL` — the payload is POSTed as JSON and the JSON reply becomes the result (`via: rail:http`). See `.env.example`.
...
| Advise, ranked by ₹ impact | impact × confidence × per-merchant agent weight learned from approvals |
| Execute, one tap / "haan" | approve button or voice → `actions.approve()` → executor |
...
| Data | all SQL scoped by `merchant_id`; `server/db.js` is the only driver seam | Postgres partitioned by merchant; Kafka → lakehouse → feature store feeding `analytics.js` |
...
- Marathi/Gujarati/Tamil/Bengali text is my own translation — please have native speakers review it.
